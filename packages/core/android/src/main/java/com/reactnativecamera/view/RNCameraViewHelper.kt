...
object RNCameraViewHelper {
  ...
  fun emitFacesDetectedEvent(view: ViewGroup, data: WritableArray?) {
    val reactContext: ReactContext = view.getContext() as ReactContext
    reactContext.runOnNativeModulesQueueThread(Runnable {
      val event: FacesDetectedEvent = FacesDetectedEvent.obtain(view.getId(), data)
      reactContext.getNativeModule(UIManagerModule::class.java).getEventDispatcher().dispatchEvent(event)
    })
  }

  fun emitFaceDetectionErrorEvent(view: ViewGroup, faceDetector: RNFaceDetector?) {
    val reactContext: ReactContext = view.getContext() as ReactContext
    reactContext.runOnNativeModulesQueueThread(Runnable {
      val event: FaceDetectionErrorEvent = FaceDetectionErrorEvent.obtain(view.getId(), faceDetector)
      reactContext.getNativeModule(UIManagerModule::class.java).getEventDispatcher().dispatchEvent(event)
    })
  }

  // Barcode detection events
  fun emitBarcodesDetectedEvent(view: ViewGroup, barcodes: WritableArray?, compressedImage: ByteArray?) {
    val reactContext: ReactContext = view.getContext() as ReactContext
    reactContext.runOnNativeModulesQueueThread(Runnable {
      val event: BarcodesDetectedEvent = BarcodesDetectedEvent.obtain(view.getId(), barcodes, compressedImage)
      reactContext.getNativeModule(UIManagerModule::class.java).getEventDispatcher().dispatchEvent(event)
    })
  }

  fun emitBarcodeDetectionErrorEvent(view: ViewGroup, barcodeDetector: RNBarcodeDetector?) {
    val reactContext: ReactContext = view.getContext() as ReactContext
    reactContext.runOnNativeModulesQueueThread(Runnable {
      val event: BarcodeDetectionErrorEvent = BarcodeDetectionErrorEvent.obtain(view.getId(), barcodeDetector)
      reactContext.getNativeModule(UIManagerModule::class.java).getEventDispatcher().dispatchEvent(event)
    })
  }

  // Bar code read event
  fun emitBarCodeReadEvent(view: ViewGroup, barCode: Result?, width: Int, height: Int, compressedImage: ByteArray?) {
    val reactContext: ReactContext = view.getContext() as ReactContext
    reactContext.runOnNativeModulesQueueThread(Runnable {
      val event: BarCodeReadEvent = BarCodeReadEvent.obtain(view.getId(), barCode, width, height, compressedImage)
      reactContext.getNativeModule(UIManagerModule::class.java).getEventDispatcher().dispatchEvent(event)
    })
  }

  // Text recognition event
  fun emitTextRecognizedEvent(view: ViewGroup, data: WritableArray?) {
    val reactContext: ReactContext = view.getContext() as ReactContext
    reactContext.runOnNativeModulesQueueThread(Runnable {
      val event: TextRecognizedEvent = TextRecognizedEvent.obtain(view.getId(), data)
      reactContext.getNativeModule(UIManagerModule::class.java).getEventDispatcher().dispatchEvent(event)
    })
  }
    ...
    for (tagInfo in exifTags) {
      ...
      if (exifMap.hasKey(name)) {
        ...
        when (type) {
          ...
          "double" -> {
            exifInterface.setAttribute(name, java.lang.Double.toString(exifMap.getDouble(name)))
